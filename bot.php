...
final class StatsBot {
    ...
    function __destruct() {
        foreach($this->channels as $channel) {
            $this->send('PART ' . $channel . ' :StatsBot out');
        }
    ...
    function mainLoop() {
        ...
        if($bufferParts[1] === '002') { // connection established keyword
            ...
            if(isset($this->settings['oper'])) {
                $this->send('OPER ' . $this->settings['oper']['user'] . ' ' . $this->settings['oper']['pass']);
            }
            if($this->settings['nickserv']['pass'] !== '') {
                $this->msg('NickServ', 'IDENTIFY ' . $this->settings['nickserv']['nick'] . ' ' . $this->settings['nickserv']['pass']);
            }
            foreach($this->channels as $channel) {
            ...
            $this->channels = array_diff($this->channels, [$bufferParts[2]]);
            $this->saveChannels();
        } elseif(strtolower($bufferParts[1]) === 'privmsg' && $bufferParts[2] === $this->nick && in_array(':help', $bufferParts)) {
    ...
    function saveChannels() {
        ...
        foreach($this->channels as $channel) {
            if(!is_dir('logs/' . trim($channel))) {
                mkdir('logs/' . trim($channel));
            }
        }
    ...
    function logLine($buffer, $bufferParts, $nick, $channel) {
        if((@substr($bufferParts[2], 0, 1) !== '#' && @substr($bufferParts[2], 0, 2) !== ':#') && strtolower($bufferParts[1]) !== 'nick') {
            return;
        } //not in channel, not nick
